...
EXPECTED_ACTION_KEYS = {"title", "detail", "priority"}
...
def test_ios_contract_has_expected_snake_case_shape():
    response = client.post(
        "/api/v1/analyze",
        json={"text": "상대가 내 영상을 유포하겠다고 협박하고 있어요."},
    )
    ...
    priorities = [item["priority"] for item in result["immediate_actions"]]
    ...
    for agency in result["recommended_agencies"]:
        ...
        parsed = urlparse(agency["website"])
